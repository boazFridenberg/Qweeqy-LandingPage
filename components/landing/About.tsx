...
export function About() {
  return (
    <section id="about" className="px-5 py-16 sm:px-8 sm:py-24">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-[2rem] border border-pearl/10 bg-surface p-8 sm:p-12">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3">
          {[
            ...
            <div key={item.label} className="rounded-2xl border border-pearl/10 bg-inset px-5 py-4">
              <p className="text-2xl font-semibold text-gold">{item.stat}</p>
              <p className="mt-1 text-sm text-pearl/75">{item.label}</p>
            </div>
          ))}
        </div>
      </div>
